...
var playAgian = document.createElement("button");
playAgian.id = "btn";
playAgian.innerText = "Play Again";
playAgian.onclick = reloadPage
game.appendChild(playAgian);
var div = [];
var span = [];
var x = false;
var o = false;
...
function userInput1() {
  swal("Welcome to Tick Tack Toe","Player1: Please Select '0' or 'X' to continue", {
    buttons: {
      // cancel: "Run away!",
      catch: {
        text: "O",
...
function winLose(name) {
  ...
  if (span[1].innerText !== "" && span[2].innerText !== "" && span[3].innerText !== "" && span[1].innerText === span[2].innerText && span[1].innerText === span[3].innerText && span[2].innerText === span[3].innerText) {
    turns = [];
    var winner = document.getElementById("span_1").innerText;
    swal("Congratulations! ", "Player " + winner + " wins");
  }
